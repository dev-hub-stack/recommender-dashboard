        ...
        <div className="flex flex-col items-start gap-3 w-full">
          {growthToolsItems.map((item, index) => (
            <Button
              key={index}
              onClick={() => {
                // Handle Cross-Selling navigation, show coming soon for others
              ...
              variant={activeView === "Cross-Selling" && item.label === "Cross-Selling" ? "default" : "ghost"}
              className={`flex items-center justify-start gap-2.5 px-2.5 py-2 w-full h-auto rounded-lg ${
                activeView === "Cross-Selling" && item.label === "Cross-Selling"
                  ? "bg-foundation-blueblue-900 hover:bg-foundation-blueblue-900"
                  : "bg-transparent hover:bg-foundation-greygrey-50"
              }`}
            >
              <img className="w-5 h-5" alt={item.label} src={item.icon} />
              <span className={`flex-1 text-left [font-family:'Poppins',Helvetica] font-normal text-sm tracking-[0] leading-[normal] ${
                activeView === "Cross-Selling" && item.label === "Cross-Selling" ? "text-brandwhite" : "text-grey-900"
              }`}>
                {item.label}
              </span>
            </Button>
